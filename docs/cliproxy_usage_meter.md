...
支持与其他项目共用 Python 3.14，运行时仅依赖标准库，无需单独虚拟环境。
使用 pyenv 时，启动命令中的 `python3` 应通过 pyenv shim 跟随全局版本；tmux 等
常驻启动命令也应使用此入口，避免固定到旧版解释器的安装路径。
`QLAB_PYTHON_BIN` 仅在需要显式覆盖解释器时设置。
...
Sub2API 返回结构化工作区和成员信息时，直接使用与桌面端 auth 一致的订阅身份；缺少工作区
时，唯一邮箱匹配也会包含独立登录的 Codex 本地 home。同邮箱对应多个工作区时不猜测归属。
旧 Sub2API 独立身份的历史用量与额度快照会在账号清单更新前幂等迁移，窗口消费因此包含
桌面端及 Sub2API 的已采集记录。卡片的 `S` 表示 Sub2API 提供额度信号，不表示所有调用都经过它。
...
完整的已有记录补价，并安全校正能够证明原先按短档冻结的历史长上下文记录。具有长上下文
价格档的模型按每次调用完整 `input_tokens` 判断：`<=272,000` 使用短档，`>272,000` 使用长档；`cached_tokens`
...
`gpt-6-astra` 是例外：所有上下文长度都使用同一费率，不收取输入/缓存 2 倍、输出
1.5 倍的长上下文附加费；cache write 仍按原有短档费率计算。启动及价格同步会清除旧版
...
列表在账号旁显示 Sub2API 原始 `session_id` 的末 4 位字符（保留字母和前导零），并标出采集来源。
仅此后缀保存在 `session_id_suffix`，完整 ID 不落库；未提供会话标识的来源及历史记录显示 `—`。
Sub2API 正常轮询会补齐重叠窗口内已有记录的后缀，不重复累计调用和费用。
同一后缀可能碰撞，且一条记录仅代表该次已完成调用使用的订阅，不表示会话被永久绑定到该账号。
Sub2API 与 Codex 本地导入没有真实 HTTP 状态码，列表显示推定的“成功”，不把内部用于统计的
`200` 当作上游实测状态；这些来源不提供完整失败重试历史。计费档沿用逐记录的实际等级、请求等级
和等级来源；缺失的等级仍显示“模式未知”。
...
3. `5% <= used_percent < 100%`：按 `窗口累计成本 / used_percent * 100` 做比例投影；其中
   ...
   卡片直接展示“本周/月已采集金额 ÷ 已用百分比”，该金额与跨周期的“累计消费”分别显示。
...
为了最小化关联数据，完整 session/thread/turn/request ID 不写入 SQLite；当前只从 Sub2API
显式提供的 `session_id` 提取末 4 位，随账号逐调用明细一同退役清理，不进入匿名累计统计。
